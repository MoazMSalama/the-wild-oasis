...
const StyledSalesChart = styled(DashboardBox)`
  grid-column: 1 / -1; // occupy the full width from the beginning (1) to the end (-1)
...
function SalesChart({ bookings, numDays }) {
  ...
  const data = allDates.map((date) => {
    return {
      ...
      totalSales: bookings
        ...
        .reduce((acc, cur) => acc + cur.totalPrice, 0), // ? why reduce try another logic
  ...
  return (
    <StyledSalesChart>
      <Heading as="h2">
        Sales from {format(allDates.at(0), "MMM ddd yyyy")} &mdash;{" "}
